...
class EmailViewer(QWidget):
    def apply_filter_to_emails(self, regex):
        """
        Filter emails in the table by the given regex (applies to the 'From' field and possibly subject).
        """
        if not regex:
            # If no regex, show all emails
            ...
            try:
                ...
                return
            filtered_emails = [email for email in self.emails_data if pattern.search(str(email.get('from', ''))) or pattern.search(str(email.get('subject', '')))]
